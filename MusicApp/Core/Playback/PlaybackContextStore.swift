...
@MainActor
final class PlaybackContextStore: ObservableObject {
    @Published private(set) var session: PlaybackContext?
    
    var currentTrackID: UUID? { session?.currentTrackID }
    
    func setSession(_ session: PlaybackContext?) {
